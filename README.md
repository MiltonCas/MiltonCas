# 👋 Hola, soy Milton Aldair Salgado Casiano

🚀 Ingeniero en Telemática egresado de UPIITA-IPN
🤖 Desarrollador IoT e Inteligencia Artificial
📡 Enfocado en Sistemas Embebidos, Visión por Computadora y Comunicación RF

## 🧰 Tecnologías Principales

![Arduino](https://img.shields.io/badge/Arduino-00979D?style=for-the-badge\&logo=Arduino\&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge\&logo=python\&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge\&logo=cplusplus\&logoColor=white)
![MATLAB](https://img.shields.io/badge/MATLAB-orange?style=for-the-badge)
![Raspberry Pi](https://img.shields.io/badge/RaspberryPi-A22846?style=for-the-badge\&logo=raspberry-pi\&logoColor=white)
![SQL Server](https://img.shields.io/badge/SQL--Server-CC2927?style=for-the-badge&logo=microsoftsqlserver&logoColor=white)

---

## 🚀 Proyectos Destacados

📦 **Contenedor Inteligente con IA**
Sistema con YOLOv8 para clasificación de residuos

📡 **Comunicación RF con Corrección de Errores**
CRC, Hamming (7,4) y Codificación Convolucional

📊 **Análisis de Señales en MATLAB**
FFT, Modulación AM/FM/PM y espectros

---

## 📫 Contacto

* LinkedIn: (tu link aquí)
